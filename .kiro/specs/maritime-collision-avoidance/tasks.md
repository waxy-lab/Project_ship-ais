...
- [ ] 10. 集成避碰决策模块到ROS2系统
  ...
  - [x] 10.2 实现避碰决策主循环
    ...
    - 实现完整的避碰决策流程
    - 新增多威胁综合规划、决策节流、时间戳、状态机日志
    - _Requirements: 3.1-3.6, 4.1-4.5, 5.1-5.6_

  - [x] 10.3 定义控制指令消息类型
    - 创建 ControlCommand.msg 消息定义
    - 包含航向调整、速度调整等字段
    - 在节点中添加 /collision_avoidance/control_commands 发布器
    - _Requirements: 5.5_

  - [x] 10.4 修改AIS模拟器以接收控制指令
    - 在 ais_sim_node.py 中添加控制指令订阅器
    - 应用控制指令到船舶状态更新逻辑
    - 替换现有的简单避碰逻辑
    - 新增 /ais/ship_states 发布器供避碰节点订阅
    - _Requirements: 5.5_

  - [x] 10.5 编写集成测试
    - 测试完整的避碰流程
    - 从场景生成到避让完成
    - 24个测试用例全部通过（对遇/追越/交叉/多威胁/安全/返航/一致性）
    - _Requirements: 1.1-1.5, 3.1-3.6, 4.1-4.5, 5.1-5.6_

- [x] 11. Checkpoint - 避碰决策模块验证
  - 运行集成测试 ✓ 207个测试全部通过
  - 手动运行几个场景验证避碰效果 ✓
  - 确保所有测试通过，询问用户是否有问题
...
- [ ] 12. 实现测试评估框架
  ...
  - [x] 12.3 实现性能指标计算
    - 计算避碰成功率、平均距离等指标
    - 创建 metrics_calculator.py
    - _Requirements: 6.1-6.5_

  - [x] 12.4 编写性能指标计算的单元测试
    - 测试各项指标的计算逻辑
    - 43个测试用例全部通过
    - _Requirements: 6.1-6.5_
...
- [x] 14. Checkpoint - 数据记录和评估验证
  - [x] 运行完整的场景，验证数据记录（5/5场景通过）
  - [x] 检查导出的数据文件（JSON/CSV/TXT均正常）
  - [x] 验证性能报告生成（成功率100%, COLREGS遵守率80%）
  - [x] 确保所有测试通过（207+61+25=293个测试全部通过）

- [x] 15. 可选功能扩展（MVP后）
  - [x] 15.1 实现多船包围场景生成器
    - 生成目标船被多艘船舶包围的场景
    - SurroundingParams + generate_surrounding_scenario()
    - 24个测试用例全部通过
    - _Requirements: 2.2_
  ...
  - [x] 15.4 实现能见度不良场景生成器
    - 调整碰撞风险阈值（预警0.3/危险0.5，低于正常0.5/0.7）
    - 增大安全距离(3nm)和安全时间(15min)
    - PoorVisibilityParams + generate_poor_visibility_scenario()
    - 38个测试用例全部通过
    - _Requirements: 2.5_
  ...
  - [x] 15.7 实现配置热加载功能
    - 监听配置文件变化（基于mtime轮询）
    - 自动重新加载参数并通知回调
    - ConfigWatcher + MultiConfigWatcher
    - 32个测试用例全部通过
    - _Requirements: 8.1_

...
from collision_avoidance.rules_engine import (
    determine_encounter_type, apply_colregs_rule, apply_restricted_water_rule,
    EncounterType, ActionType, TurnDirection, AvoidanceAction,
    is_overtaking_complete, is_avoidance_complete,
)
...
SCENARIOS = {
    'head_on_basic': {
        'label': '基础对遇',
        'desc': '两艘船各以20节相向航行，初始距离1.5nm，触发Rule 14右转15°避让后回正',
        'env': {'天气': '平静', '能见度': '良好'},
    },
    'head_on_high_speed': {
        'label': '高速对遇',
        'desc': '两艘船各以30节高速相向，初始距离2nm，高CRI场景测试紧迫避让响应',
        'env': {'天气': '平静', '能见度': '良好'},
    },
    'overtaking': {
        'label': '追越避让',
        'desc': '本船40节从后方追越12节慢船，初始距离0.8nm，Rule 13右转绕行后回正',
        'env': {'天气': '平静', '能见度': '良好'},
    },
    'surrounding': {
        'label': '多船包围',
        'desc': '本船被4艘18-22节目标船从四周1.5nm处包围，测试多威胁优先级决策',
        'env': {'天气': '平静', '能见度': '良好'},
    },
    'rough_weather': {
        'label': '恶劣天气对遇',
        'desc': '15m/s大风+1.5m/s海流，两船各20节对遇，风流偏航扰动+额外10°转向余量（Rule 14）',
        'env': {'天气': '恶劣', '风速': '15 m/s', '流速': '1.5 m/s'},
    },
    'poor_visibility': {
        'label': '能见度不良',
        'desc': '能见度1nm大雾，两船各15节对遇，Rule 19减速70%+大幅右转30°保守避让',
        'env': {'天气': '大雾', '能见度': '1.0 nm'},
    },
    'restricted_water': {
        'label': '受限水域',
        'desc': '本船15节沿90°航道行驶，目标从右前方1nm处以225°斜向驶来，Rule 9减速靠右避让',
        'env': {'水域': '狭水道', '航道宽': '0.8 nm'},
    },
}
...
def _build_scenario(name: str) -> ScenarioConfig:
    if name == 'head_on_basic':
        return gen.generate_head_on_scenario(
            HeadOnParams(distance=1.5, speed1=20.0, speed2=20.0))
    if name == 'head_on_high_speed':
        return gen.generate_head_on_scenario(
            HeadOnParams(distance=2.0, speed1=30.0, speed2=30.0))
    if name == 'overtaking':
        ...
        scenario.ships[0] = dataclasses.replace(s0,
            latitude=30.0, longitude=120.0 - dist_deg, heading=90.0, sog=40.0)
        scenario.ships[1] = dataclasses.replace(s1,
            latitude=30.0, longitude=120.0, heading=90.0, sog=12.0)
        return scenario
    if name == 'surrounding':
        # 包围距离缩小，包围船速加快，确保150s内形成威胁
    ...
    if name == 'rough_weather':
        return gen.generate_rough_weather_scenario(
            RoughWeatherParams(wind_speed_ms=15.0, wind_direction=45.0,
                               ...
                               visibility_nm=3.0,
                               distance_nm=1.2,
                               own_speed=20.0, target_speed=20.0))
    if name == 'poor_visibility':
        return gen.generate_poor_visibility_scenario(
            PoorVisibilityParams(visibility_nm=1.0, distance_nm=1.2,
                                 own_speed=15.0, target_speed=15.0,
                                 risk_threshold_warning=0.3,
                                 risk_threshold_danger=0.5))
    if name == 'restricted_water':
        scenario = gen.generate_restricted_water_scenario(
            RestrictedWaterParams(channel_width_nm=0.8, channel_length_nm=6.0,
                                  channel_heading=90.0,
                                  own_speed=15.0, target_speed=12.0,
                                  separation_nm=1.0))
        import dataclasses as _dc, math as _math
        own = scenario.ships[0]
        tgt = scenario.ships[1]
        cos_lat = max(_math.cos(_math.radians(own.latitude)), 1e-6)
        NM = 1.0 / 60.0
        # 本船：强制沿航道向东（90°），置于航道西端中心线上
        scenario.ships[0] = _dc.replace(own,
            latitude=own.latitude,
            longitude=own.longitude,
            heading=90.0)
        # 目标：置于本船正东1.0nm处，偏北0.1nm（右舷前方），航向225°斜向驶来
        scenario.ships[1] = _dc.replace(tgt,
            latitude=own.latitude + 0.1 * NM,
            longitude=own.longitude + 1.0 * NM / cos_lat,
            heading=225.0)
        return scenario
    # 默认
...
def _assess_all(ships: List[dict]):
    ...
    for i, tgt in enumerate(ships[1:], 1):
        try:
            ...
            # 根据水域类型选择规则：受限水域用 Rule 9，开阔水域用标准 COLREGS
            with _sim_lock:
                is_restricted = bool(_sim_state.get('map_polygon'))
                env_obj = _sim_state.get('environment_obj')  # EnvironmentConfig 对象
            if is_restricted:
                action = apply_restricted_water_rule(own_state, tgt_state)
            else:
                action = apply_colregs_rule(enc, own_state, tgt_state, environment=env_obj)
            ...
            if ra.risk_level.value in ('warning', 'danger'):
                ...
                elif action.action_type == ActionType.SPEED_CHANGE and action.speed_factor:
                    # 受限水域减速：直接调整本船速度
                    if own.get('avoidance_cooldown', 0) == 0:
                        orig_sog = own.get('original_sog', own['sog'])
                        own['original_sog'] = orig_sog
                        new_sog = round(orig_sog * action.speed_factor, 1)
                        if own['sog'] != new_sog:
                            own['sog'] = new_sog
                            own['avoidance_cooldown'] = AVOID_COOLDOWN_AFTER_TURN
                            events.append(
                                f"[减速] MMSI {own['mmsi']} → {new_sog:.1f}节 ({action.reason})"
                            )
            ...
            if (action.action_type == ActionType.RESUME_COURSE
                    and own.get('original_sog') is not None
                    and own['sog'] < own['original_sog']):
                own['sog'] = own['original_sog']

            # 避让完成回正：规则引擎返回 RESUME_COURSE（追越/对遇/交叉均适用）
            # 不受 avoidance_cooldown 限制（回正是主动安全恢复动作）
            if (action.action_type == ActionType.RESUME_COURSE
                    and own.get('target_heading') is None):
                ...
                if orig is not None:
                    ...
                    if diff > 2.0:
                        ...
                        own['avoidance_cooldown'] = 0
                        events.append(
                            f"[回正] MMSI {own['mmsi']} 避让完成，回正至原始航向 {orig:.0f}° ({action.reason})"
                        )
        except Exception:
            pass
...
def _sim_loop():
    ...
    while True:
        ...
        # 风流偏航扰动（恶劣天气/能见度不良场景专有效果）
        with _sim_lock:
            env_obj = _sim_state.get('environment_obj')
        if env_obj is not None:
            from scenario_generator.models import WeatherCondition, Visibility
            wind_spd = getattr(env_obj, 'wind_speed', 0.0)
            wind_dir = getattr(env_obj, 'wind_direction', 0.0)
            cur_spd  = getattr(env_obj, 'current_speed', 0.0)
            cur_dir  = getattr(env_obj, 'current_direction', 0.0)
            wc  = getattr(env_obj, 'weather_condition', None)
            vis = getattr(env_obj, 'visibility', None)

            # 风的偏航影响（风速 m/s 转换为漂移节：1 m/s ≈ 2节）
            wind_drift_kn = wind_spd * 0.15  # 约15%风速作用于船体侧漂
            cur_drift_kn  = cur_spd  * 1.944 * 0.8  # 流速80%传递

            for s in ships:
                drift_deg = 0.0
                # 风漂：风向与船首角度差决定偏航方向
                if wind_spd > 3.0:
                    rel_wind = (wind_dir - s['heading']) % 360
                    # 右舷来风向右偏，左舷来风向左偏
                    drift_deg += wind_drift_kn * math.sin(math.radians(rel_wind)) * dt / 60.0
                # 流漂：流向与船首角度差
                if cur_spd > 0.1:
                    rel_cur = (cur_dir - s['heading']) % 360
                    drift_deg += cur_drift_kn * math.sin(math.radians(rel_cur)) * dt / 60.0
                # 只对无避让指令的船施加漂移（避让中不额外扰动）
                if abs(drift_deg) > 0.01 and s.get('target_heading') is None:
                    s['heading'] = (s['heading'] + drift_deg) % 360
...
def api_start():
    ...
    SCENE_DURATION = {
        'head_on_basic':    180,
        'head_on_high_speed': 160,
        'overtaking':       300,
        'surrounding':      180,
        'rough_weather':    280,
        'poor_visibility':  280,
        'restricted_water': 300,
    }
    ...
    with _sim_lock:
        _sim_state.update({
            ...
            'environment_obj': scenario.environment,  # 原始对象，供规则引擎使用
